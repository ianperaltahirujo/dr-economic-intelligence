...
<head>
    ...
    <style>
        *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        .hero-video-wrap {{ position: absolute; inset: 0; -webkit-mask-image: linear-gradient(to top, transparent, black 28%); mask-image: linear-gradient(to top, transparent, black 28%); }}
        ...
        .hero-editorial {{ padding: clamp(28px, 4vw, 44px) clamp(16px, 5vw, 40px) clamp(40px, 7vw, 72px); border-bottom: var(--border); }}
...
.alert-count {{ display: inline-flex; align-items: center; gap: 8px; margin-top: 18px; padding: 10px 16px; border-radius: 999px; background: {'var(--red-tint)' if stress_n > 0 else 'var(--gray-100)'}; border: 1px solid {'var(--red)' if stress_n > 0 else 'var(--gray-200)'}; font-size: 13px; color: {'var(--red)' if stress_n > 0 else 'var(--gray-600)'}; font-family: var(--font-mono); align-self: flex-start; }}        .interactive-alert {{ cursor: pointer; transition: all .2s var(--ease); }}
        ...
        /* Month gauge docked beside the chart (updates on hover, defaults to latest month); divider resizes chart vs gauge */
        .chart-layout {{ display: flex; align-items: stretch; }}
        .chart-layout .chart-container {{ flex: 1 1 auto; min-width: 0; }}
        .chart-divider {{ flex: 0 0 auto; width: 20px; align-self: stretch; position: relative; display: flex; align-items: center; justify-content: center; cursor: ew-resize; touch-action: none; -webkit-user-select: none; user-select: none; }}
        .chart-divider::before {{ content: ''; position: absolute; top: 0; bottom: 0; left: 50%; width: 1px; background: var(--gray-200); transform: translateX(-50%); transition: background .15s var(--ease); }}
        .chart-divider-grip {{ position: relative; z-index: 1; width: 5px; height: 34px; border-radius: 3px; background: var(--gray-200); border: 1px solid var(--gray-300); transition: background .15s var(--ease), border-color .15s var(--ease); }}
        .chart-divider:hover .chart-divider-grip, .chart-divider.dragging .chart-divider-grip {{ background: var(--gray-400); border-color: var(--gray-400); }}
        .chart-divider:hover::before, .chart-divider.dragging::before {{ background: var(--gray-300); }}
        .chart-gauge-panel {{ flex: 0 0 var(--gauge-w, 232px); overflow: hidden; display: flex; align-items: center; justify-content: center; }}
        .cg-inner {{ width: 216px; flex: 0 0 auto; display: flex; flex-direction: column; align-items: center; text-align: center; }}
        ...
        @media (max-width: 720px) {{
            ...
            .chart-gauge-panel {{ flex: none; width: 100%; padding-left: 0; border-top: var(--border); padding-top: 18px; margin-top: 4px; overflow: visible; }}
            .cg-inner {{ width: 100%; }}
        ...
        @media (max-width: 640px) {{
            ...
            /* Left-align the nav and reserve space for the fixed theme toggle so the
               "Clima Social" pill never sits under it. Flatten the nested header-nav
               scroll so the whole row scrolls as one and the full pill stays reachable. */
            .header-inner--centered {{ justify-content: flex-start; padding-right: 60px; }}
            .header-nav {{ overflow-x: visible; flex: 0 0 auto; }}
            .nav-link--clima {{ flex: 0 0 auto; }}
            .nav-labels {{ padding-bottom: 2px; padding-right: 14px; }}
        ...
        @media print {{
            .site-header, .back-to-top, .chart-controls, .chart-hint, .filter-bar, .theme-toggle {{ display: none !important; }}
        ...
        /* Dark mode */
        [data-theme="dark"] {{
            --blue: #4A90D9; --blue-deep: #5B9BD5;
            --white: #111111; --black: #E8E8E8;
            --gray-50: #181818; --gray-100: #222222; --gray-200: #2C2C2C;
            --gray-300: #3A3A3A; --gray-400: #7A7A7A; --gray-600: #AAAAAA;
            --blue-tint: #0A1929; --blue-soft: #1B3055; --red-tint: #200A0C; --green: #5CB85C;
    ...
    <script>(function(){{var t=localStorage.getItem('dr-ei-theme');if(t==='dark')document.documentElement.setAttribute('data-theme','dark');}})();</script>
</head>
<body>
...
// ---- Draggable divider: resize chart vs gauge (drag left = default width, drag right hides the gauge) ----
(function() {{
    var divider = document.getElementById('chartDivider');
    var panel = document.querySelector('.chart-gauge-panel');
    if (!divider || !panel) return;
    var DEFAULT_W = 232, MIN_W = 0, STEP = 24;
    var gaugeW = DEFAULT_W, dragging = false, startX = 0, startW = DEFAULT_W, raf = null;
    function scheduleResize() {{ if (raf) return; raf = requestAnimationFrame(function() {{ raf = null; if (scoreChart && scoreChart.resize) scoreChart.resize(); }}); }}
    function setW(w) {{ gaugeW = Math.max(MIN_W, Math.min(DEFAULT_W, w)); panel.style.setProperty('--gauge-w', gaugeW + 'px'); scheduleResize(); }}
    function isMobile() {{ return window.matchMedia('(max-width: 720px)').matches; }}
    function startDrag(clientX) {{ dragging = true; startX = clientX; startW = gaugeW; divider.classList.add('dragging'); document.body.style.userSelect = 'none'; }}
    function moveDrag(clientX) {{ if (!dragging) return; setW(startW - (clientX - startX)); }}
    function endDrag() {{ if (!dragging) return; dragging = false; divider.classList.remove('dragging'); document.body.style.userSelect = ''; }}
    divider.addEventListener('mousedown', function(e) {{ if (isMobile()) return; startDrag(e.clientX); e.preventDefault(); }});
    window.addEventListener('mousemove', function(e) {{ moveDrag(e.clientX); }});
    window.addEventListener('mouseup', endDrag);
    divider.addEventListener('touchstart', function(e) {{ if (isMobile()) return; startDrag(e.touches[0].clientX); }}, {{ passive: true }});
    window.addEventListener('touchmove', function(e) {{ if (dragging) moveDrag(e.touches[0].clientX); }}, {{ passive: true }});
    window.addEventListener('touchend', endDrag);
    divider.addEventListener('keydown', function(e) {{
        if (e.key === 'ArrowLeft') {{ setW(gaugeW + STEP); e.preventDefault(); }}
        else if (e.key === 'ArrowRight') {{ setW(gaugeW - STEP); e.preventDefault(); }}
    }});
}})();
...
(function() {{
    ...
    // Keep the active nav link in view: horizontally scroll the nav's scroll container
    // so the current section's link is centered as the page scrolls. The scrollable
    // element differs by page (.nav-labels when flattened, else .header-nav), so walk up
    // from the link to find the nearest horizontally-scrollable ancestor. scrollTo on
    // that element only -- never scrollIntoView, which would also move the page vertically.
    function revealActive(idx) {{
        if (idx < 0 || idx >= links.length) return;
        var link = links[idx];
        var sc = link.parentElement;
        var stop = cluster.parentElement;
        while (sc && sc !== stop && sc.scrollWidth <= sc.clientWidth + 1) {{ sc = sc.parentElement; }}
        if (!sc || sc === stop || sc.scrollWidth <= sc.clientWidth + 1) return;
        var sRect = sc.getBoundingClientRect();
        var lRect = link.getBoundingClientRect();
        var delta = (lRect.left - sRect.left) - (sc.clientWidth - lRect.width) / 2;
    ...
    function update() {{
        ...
        if (activeIdx !== lastActive) {{ lastActive = activeIdx; revealActive(activeIdx); }}
    }}
